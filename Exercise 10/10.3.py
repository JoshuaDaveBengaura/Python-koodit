...
class Building():
    ...
    def fire_alarm(self):
        elevator_number = 1
        print(f"FIRE ALERT! ALL ELEVATORS ARE NOW HEADING TO FLOOR {self.bottom_floor}")
        print()
        for a in self.building_elevators:
            print(f"ELEVATOR NUMBER: {elevator_number}")
            a.go_to_floor(a.bottom_floor)
            print()
            elevator_number += 1
